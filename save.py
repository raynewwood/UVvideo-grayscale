#version 1.1.1
#change time 2021 07 13 21:04
#developed by Leilei Niu
import numpy as np
import cv2
import math
import datetime
...
out = cv2.VideoWriter('C_T_%s-N_%d-Th_%d.mp4'%(input,nihe,thresh_v), cv2.VideoWriter_fourcc(*'XVID'), 25.0, size,1)#C_T_%s-N_%d-Th_%d.jpg'%(input,nihe,thresh_v)
threshold=size[0]*size[1]/1000#小面积消除阈值
f=0
count=1
...
while(cap.isOpened()):
    ...
    file__ = open('C_T_%s_N_%d_Thb_%d_Thl%d.txt' % (input, nihe, thresh_v, threshold), 'a')
    ...
    if nihe ==1: #圆拟合
        ...
        if ret==True:
            ...
            ret, thresh = cv2.threshold(gray, thresh_v, 255, 0)  # 二值化阈值
            ...
            for c in range(len(contours)):
                area_contour =cv2.contourArea(contours[c])

                if area_contour< threshold:
                    continue

                (x, y), radius = cv2.minEnclosingCircle(contours[c])
                frame_area += area_contour
                frame_are_nihe +=np.pi*radius*radius
                center = (int(x), int(y))
                radius = int(radius)
            ...
            file__.write(str(k))
            file__.write(' ')
            file__.write(str(frame_area))
            file__.write(' ')
            file__.write(str(frame_are_nihe))
            file__.write('\n')
        ...
        if ret==True:
            ...
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)#灰度化
            ret, thresh = cv2.threshold(gray, thresh_v, 255, 0)  # 二值化
            kernel = np.ones((5, 5), np.uint8)
            opening = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)#开运算
            closing = cv2.morphologyEx(opening, cv2.MORPH_CLOSE, kernel)#闭运算
            contours, hierarchy = cv2.findContours(closing, 3, 1)#轮廓检测
...
cv2.imwrite('C_T_%s-N_%d-Thb_%d-M%d-Thl%d.jpg'%(input,nihe,thresh_v,M,threshold), color)#C T N Thb m Thl分别表示彩色，视频时长，拟合类型，灰度化阈值，矩阵最大值，小面积消除阈值
cap.release()
out.release()
